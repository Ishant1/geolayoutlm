...
def main():
    ...
    save_class_names()
    shutil.copytree(os.path.join(INPUT_PATH, 'training_data'), os.path.join(OUTPUT_PATH, 'training_data'))
    shutil.copytree(os.path.join(INPUT_PATH, 'testing_data'), os.path.join(OUTPUT_PATH, 'testing_data'))
...
def do_preprocess(tokenizer, dataset_split):
    ...
    for json_file in tqdm(json_files):
        ...
        out_json_obj["meta"] = {}
        image_file = (
            os.path.splitext(json_file.replace(f"{anno_dir}", "images"))[0] + ".jpeg"
        )
        if dataset_split == "train":
